...
// PUT -> UPATE ENTIRE VALUE -> { name : 'ale', age : '19' } -> PUT ({ name : 'alfon' }) -> result => { name : 'alfon' }
// PATCH -> PARTIAL UPDATE -> { name : 'ale', age : '19' } -> PATCH ({ name : 'alfon' }) -> result => { name : 'alfon', age : '19'}
app.patch('/products/:id', (req, res) => {
    ...
    // if failed
    if (index === undefined) {
        return res.status(400).send(`Bad Request : product with id : ${id} doesn't found.`)
    }

    // if exist -> do update
    products[index] = Object.assign(products[index], body)
